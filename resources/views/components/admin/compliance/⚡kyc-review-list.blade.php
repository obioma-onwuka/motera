...
new class extends Component
{
    ...
    {
        return [
            'submissions' => KycSubmission::with(['user', 'documents'])->latest()->get(),
        ];
    }
            ...
            <tbody class="divide-y divide-brand-border">
                @foreach($submissions as $sub)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 rounded-full text-[8px] font-bold uppercase {{ $sub->status === 'approved' ? 'bg-green-100 text-green-700' : ($sub->status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700') }}">
                                {{ $sub->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
        ...
        <div class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
                <div class="p-6 border-b border-brand-border flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold">Reviewing Submission</h3>
                    ...
                    <button wire:click="$set('selectedSubmission', null)" class="text-slate-400 hover:text-slate-600">
                            ...
                            @if($selectedSubmission->status === 'pending')
                                <div class="bg-white p-6 rounded-2xl border border-brand-border shadow-sm space-y-4">
                                    ...
                                    <div class="flex gap-4">
                                        <button wire:click="reject" class="flex-1 px-4 py-3 bg-red-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-red-100 hover:bg-red-700 transition-colors">Reject Submission</button>
                                        <button wire:click="approve" class="flex-1 px-4 py-3 bg-green-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-green-100 hover:bg-green-700 transition-colors">Approve KYC</button>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="space-y-6">
                            @foreach($selectedSubmission->documents as $doc)
                                <div class="bg-white p-4 rounded-2xl border border-brand-border shadow-sm space-y-2">
                                    ...
                                    <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="block rounded-lg overflow-hidden border border-brand-border">
                                        <img src="{{ Storage::url($doc->file_path) }}" class="w-full h-auto max-h-64 object-contain">
                                    </a>
                                </div>
                            @endforeach
                        </div>
